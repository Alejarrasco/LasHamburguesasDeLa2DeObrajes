<template>
    ...
    <label for="nClusters" class="block text-sm font-medium text-gray-700">Número de clusters</label>
    <input type="number" id="nClusters" v-model="nClusters" class="border border-gray-300 rounded p-2" min="1" placeholder="Número de clusters (default: 3)" />
</div>
  ...
  <img src="https://64.media.tumblr.com/be3ce7e4703c3e208790c3ed4249db99/0c7447b1cf7154b1-c4/s640x960/14fd1548c126d24b96e4731647c868ed26f1fdfe.gif" alt="Cargando..." class="h-16 w-16">
      ...
      <div v-if="clustersImage" class="mt-4 w-full">
        <ZoomImage :src="clustersImage" />
      </div>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const nClusters = ref<number>(3);
      const fileRef = ref<File | null>(null);
      const clustersImage = ref<string | null>(null);
      ...
      const submitData = () => {
        if (fileRef.value && selectedColumn.value) {
          ...
          const url = `http://localhost:8000/kmeans_clusters/?target_column=${encodeURIComponent(selectedColumn.value)}&n_clusters=${nClusters.value}`;
  
          axios.post(url, formData)
          .then(response => {
            clustersImage.value = `data:image/png;base64,${response.data.clusters_image}`;
          })
          .catch(error => {
      ...
      return { columns, selectedColumn, nClusters, handleFileUpload, submitData, clustersImage, isLoading, errorMessage };
    }
  });
  </script>

<template>
    <div class="flex flex-col items-center justify-center p-8 bg-white rounded-lg shadow-xl">
      ...
      <div v-if="selectedColumn" class="mt-4">
        <input type="number" v-model="nEstimators" class="border border-gray-300 rounded p-2" min="1" max="200" placeholder="Número de estimadores (default: 100)" />
      </div>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const nEstimators = ref<number>(100);
      ...
      const submitData = () => {
        if (fileRef.value && selectedColumn.value) {
          ...
          const url = `http://localhost:8000/random-forest/?target_column=${encodeURIComponent(selectedColumn.value)}&n_estimators=${nEstimators.value}`;
      ...
      return { columns, selectedColumn, nEstimators, handleFileUpload, submitData, report, isLoading, errorMessage };
    }
  });
  </script>

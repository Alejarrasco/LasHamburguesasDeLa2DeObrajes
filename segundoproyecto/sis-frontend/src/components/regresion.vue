<template>
    <div class="flex flex-col items-center justify-center p-8 bg-white rounded-lg shadow-xl">
      ...
      <div v-if="decisionBoundary" class="mt-4 w-full">
        <img :src="'data:image/png;base64,' + decisionBoundary" alt="Decision Boundary" />
      </div>
  ...
  export default defineComponent({
    name: 'LogisticRegressionUploader',
    components: {
      DocumentIcon
    },
    setup() {
      ...
      const decisionBoundary = ref<string | null>(null);
      ...
      const submitData = () => {
        if (fileRef.value && selectedColumn.value) {
          ...
          const url = `http://localhost:8000/logistic-regression/?target_column=${encodeURIComponent(selectedColumn.value)}`;
  
          axios.post(url, formData)
          .then(response => {
            report.value = atob(response.data.report);
            decisionBoundary.value = response.data.decision_boundary ? atob(response.data.decision_boundary) : null;
          })
          .catch(error => {
      ...
      return { columns, selectedColumn, handleFileUpload, submitData, report, decisionBoundary, isLoading, errorMessage };
    }
  });
  </script>
